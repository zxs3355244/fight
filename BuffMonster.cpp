...
BuffMonster* BuffMonster::create(string name, int id, bool isBoss, int wave,bool showEff)
{
	BuffMonster* monster = new BuffMonster();
	monster->retain();
	monster->autorelease();
	monster->mName = name;
	monster->mCurId = id;
...
bool BuffMonster::init()
{
	...
	armatureAtr->getAnimation()->setMovementEventCallFunc(CC_CALLBACK_0(BuffMonster::animationEvent, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));


	armatureOut = Armature::create("chuxian");
	this->addChild(armatureOut, 3);
	armatureHead->getBone("Layer3")->changeDisplayWithIndex(mCurId - 1, true);
	...
	armatureHead->getBone("Layer27")->changeDisplayWithIndex(mCurId - 1, true);
	armatureHead->getBone("Layer29")->changeDisplayWithIndex(mCurId - 1, true);
	armatureHead->getBone("Layer26")->changeDisplayWithIndex(mCurId - 1, true);
	armatureHead->getBone("Layer30")->changeDisplayWithIndex(mCurId - 1, true);
	armatureHead->getBone("Layer5_3")->changeDisplayWithIndex(mCurId - 1, true);
	armatureHead->getBone("Layer5_2")->changeDisplayWithIndex(mCurId - 1, true);
	armatureHead->getBone("Layer5")->changeDisplayWithIndex(mCurId - 1, true);
	armatureHead->getBone("Layer36")->changeDisplayWithIndex(mCurId - 1, true);
	armatureHead->getBone("Layer24")->changeDisplayWithIndex(mCurId - 1, true);
	armatureHead->getBone("Layer23")->changeDisplayWithIndex(mCurId - 1, true);
	armatureHead->getBone("Layer33")->changeDisplayWithIndex(mCurId - 1, true);
	armatureHead->getBone("Layer35")->changeDisplayWithIndex(mCurId - 1, true);
	armatureHead->getBone("Layer34")->changeDisplayWithIndex(mCurId - 1, true);

	armatureHead->getAnimation()->setMovementEventCallFunc(CC_CALLBACK_0(BuffMonster::animationEvent, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
	armatureHead->getAnimation()->setFrameEventCallFunc(CC_CALLBACK_0(BuffMonster::onFrameEvent, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4));
	...
	if (showEff)
	{
		...
		armatureOut->getAnimation()->setMovementEventCallFunc(CC_CALLBACK_0(BuffMonster::animationEvent, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
...
void BuffMonster::onFrameEvent(cocostudio::Bone *bone, const std::string& evt, int originFrameIndex, int currentFrameIndex)
{
	//CCLOG("(%s) emit a frame event (%s) at frame index (%d).", bone->getName().c_str(), evt.c_str(), currentFrameIndex);
	if (evt == "atk_frame")
	{
		ArmBullet* bullet = ArmBullet::create(ArmBulletType::mBuff, this->mCurId);
		GameLayer::getInstance()->ObjVec.push_back(bullet);
		GameLayer::getInstance()->mGameLayer->addChild(bullet);
		bullet->setPosition(Vec2(this->targetPos.x, this->targetPos.y - 10));
...
void BuffMonster::animationEvent(Armature *armature, MovementEventType movementType, const std::string& movementID)
{
...
void BuffMonster::setAction(string name, bool loop)
{
	armatureHead->getAnimation()->play(name, 0, loop);
}


void BuffMonster::showHitNum()
{
	if (mTimeNum < 1)
	{
		return;
...
void BuffMonster::BulletHit(Uint* uint)
{
...
void BuffMonster::stateHandler(int state, bool loop)
{
	if (mCurState == state)
	{
		return;
...
void BuffMonster::updateLogic(float dt)
{
	if (isDie || mCurLife < 0)
	{
		return;
...
void BuffMonster::move(Vec2 dir, float dt)
{
	if (dir.x < 0)
	{
		if (mCurDir != 2)
...
void BuffMonster::setProper(MonsterOneData* data)
{
	mSpeed = data->speed;
	mHurt = data->atk;
	mLife = data->life;
